...
namespace BioBank
{
    ...
    public partial class BioBank : Form
    {
        ...
        private void LoadCase(ComboBox cboTeamNo)
        {
            ...
            try
            {
                using (SqlConnection sCon = new SqlConnection(ClsShareFunc.DB_BIOConnection()))
                {
                    sCon.Open();
        ...
        private string GetTime()
        {
            string time;
            //撈資料庫現在時間並將西元轉民國
            using (SqlConnection conn = new SqlConnection(ClsShareFunc.DB_BIOConnection()))
            {
                conn.Open();
                SqlCommand gatTime = new SqlCommand("Select sdate = left(replace(replace(replace(CONVERT(varchar(100), GETDATE(), 120),'-',''),' ',''),':',''),12)", conn);
...
/*======================瀏覽檔案======================*/
        ...
        private void CheckWarning(DataTable dt)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (dt.Rows[i]["分庫"].ToString() != cboTeamNo.Text.Substring(1, cboTeamNo.Text.Length - 2))
        ...
        private void buttonSaveToDB_Click(object sender, EventArgs e)
        {
            ...
            if (myResult == DialogResult.Yes)
            {
              ...
              //var excelFile = new ExcelQueryFactory(textBoxFilePath.Text);
                ...
                {
                    string FileName = @"C:\\EXEC_BAT_ChihCheng\" + sExcelName + "_" + GetTime() + ".xls";
        ...
        private void SaveToDB()
        {
            ...
            using (SqlConnection conn = BioBank_Conn.Class_biobank_conn.DB_SEC_conn())
            {
                ...
                using (SqlConnection conn1 = new SqlConnection(ClsShareFunc.DB_BIOConnection()))
                {
                    conn1.Open();
                    //將bio資料庫的格式取出
        ...
        private string RandomNumber(string type, int Row)
        {
            ...
            //第一碼為小組代碼
            newNumber = cboTeamNo.Text.Substring(0, 1);
            ...
            //第七碼為檢體種類+第八碼隨機碼
            newNumber += type.Substring(0, 1) + randomtemp.Next(0, 10);
            return newNumber;
        }
                ...
                if (gfunCheckFormat(sId, sName, sPwd, sPwdVer) == true)
                {
                    ...
                    if (ClsShareFunc.CheckInDb(ClsShareFunc.DbAdmin(), sId, "insert") == false)
                    {
                        ...
                        if (ClsShareFunc.CheckInDb(ClsShareFunc.DbCom(), sId, "insert") == false)
                        {
                            ...
                            {
                                insertCon.Open();
                                insertSQL = " insert into BioCommonLoginTbl (chUserID,chUserName,chPassword,chEnableFlag,chBioEmpFlag) " +
                                    " values ('" + sId + "','" + sName + "','" + ClsShareFunc.GetMD5(sPwd) + "','N','" + sDepartment + "')";
...
/*=======================查詢/修改==========================*/
        ...
        private void btnModLReqNo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                sLReqNo = dgvShowLReqNo.Rows[0].Cells[0].Value.ToString();
                sYear = dgvShowLReqNo.Rows[0].Cells[1].Value.ToString();
                sRange = dgvShowLReqNo.Rows[0].Cells[2].Value.ToString();
                sStatus = dgvShowLReqNo.Rows[0].Cells[3].Value.ToString();
                sNote = dgvShowLReqNo.Rows[0].Cells[4].Value.ToString();
...
/*=======================出庫==========================*/
        ...
        private void dgvOutLReqNo_RowHeaderMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                {
                    if (j >= 1 && j <= 10)
                        aStr1[j - 1] = dgvOutLReqNo.Rows[sRow].Cells[j].Value.ToString();
                    else if (j >= 10 && j <= 20)
                        aStr2[j - 11] = dgvOutLReqNo.Rows[sRow].Cells[j].Value.ToString();
                    else if (j >= 20 && j <= 31)
                        aStr3[j - 21] = dgvOutLReqNo.Rows[sRow].Cells[j].Value.ToString();
                }
                ...
                if (sChkValue == "True") //有勾選就出庫
                {
                    ...
                    using (SqlConnection sCon = new SqlConnection(ClsShareFunc.DB_BIOConnection()))
                    {
                        ...
                        if (sDBTakeOutDate == "")
                        {
                            ...
                            {
                                /*3.出庫*/
                                using (SqlConnection sConOut = new SqlConnection(ClsShareFunc.DB_BIOConnection()))
                                {
                                    sConOut.Open();
...
/*======================備份======================*/
        ...
        {
            txtRemoteDestination.Text = @"\\192.168.1.3\EXEC_BAT_ChihCheng\DATA\";
            string sLocalIP = "";
        ...
        {
            txtLocalSource.Text = @"C:\EXEC_BAT_ChihCheng\DATA\";
        ...
        private void btnBackup_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection sCon = BioBank_Conn.Class_biobank_conn.DB_SEC_conn())
            {
                try
                {
                    ...
                    myProcess.StartInfo.FileName = @"C:\Batch\BackupBio-1.bat";
        ...
        private void btnRestore_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection sCon = BioBank_Conn.Class_biobank_conn.DB_SEC_conn())
            {
                try
                {
                    ...
                    updateCmd.ExecuteNonQuery();
                    updateCmd = new SqlCommand(@"RESTORE DATABASE DB_BIO FROM DISK='c:\Batch2\DB_BIO.bak' with Replace", sCon);
                    updateCmd.ExecuteNonQuery();
                    ...
                    updateCmd.ExecuteNonQuery();
                    updateCmd = new SqlCommand(@"RESTORE DATABASE DB_SEC FROM DISK='c:\Batch2\DB_SEC.bak' with Replace", sCon);
                    updateCmd.ExecuteNonQuery();
                   
                    myProcess.StartInfo.FileName = @"C:\Batch2\RestoreBio-2.bat";
                    myProcess.StartInfo.UseShellExecute = false;
                    myProcess.StartInfo.WindowStyle = ProcessWindowStyle.Hidden;
                    myProcess.Start();
                    myProcess.WaitForExit();
